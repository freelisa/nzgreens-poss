package com.nzgreens.console.util.excelutil;

import com.nzgreens.common.utils.excelutil.ExcelCell;
...
public class ExcelUtil {
  ...
  public static <T> void exportExcel(String title,String[] headers, Collection<T> dataset, HttpServletResponse res,
    ...
    try {
      ...
      res.setHeader("Content-Disposition", "attachment;filename=" + URLEncoder.encode(title, "UTF-8") + ".xls");
        ...
        if (clazz == Map.class) {
          ...
          for (FieldForSortting ffs : fields) {
            ...
            if (field.getType().isArray()) {
              ...
              String errMsg = validateCell(cell, field, cellIndex);
              //如果验证类型错误 && 类型不为String  才不进入处理
              if (field.getType().equals(String.class) || StringUtils.isBlank(errMsg)) {
                Object value = null;
                ...
                if (field.getType().equals(Date.class) )
                {
                  ...
                  {
                    Object strDate =cell.getStringCellValue();
                    try {
                  ...
                  value = processFieldTypeAndValue(field,value);
  ...
  private static Object processFieldTypeAndValue(Field field,Object value)
  {
    if(value != null)
    {
        if(field.getType().equals(String.class))
        {
          ...
          {
            value =value==null?value: specialCharacterToBlank(value.toString().trim());
          }
          return value;
        }
        if(value instanceof Double)
  ...
  public static String specialCharacterToBlank(String str) {
    String dest = "";
    if (str != null) {
