$(document).ready(function () {
    ...
    $("#btnAdd").on("click", function () {
        if ($("#addForm").valid()) {
            ...
            post(_rootPath + "product/category/insert", params, function (result) {
                $("#btnAdd").button('reset');
...
//查询列表
function searchList() {
    post(_rootPath + "product/category/search-list", $("#searchForm").serialize(), function (result) {
...
function searchDetail(agentId) {
    post(_rootPath + "product/category/search-detail", {"id": agentId}, function (result) {
        ...
        $("#updateParentId").val(result.data.parentId);
    });
}
...
function saveUpdate() {
    ...
    post(_rootPath + "product/category/update", params, function (result) {
        if (result.success == true) {
            swal({title: "提示", text: "保存成功", type: 'success'}, function () {
